...
public class RideService {
    ...
    private void notifyDrivers(Ride ride) {
        for (Driver driver : drivers.values()) {
            if (driver.getStatus() == DriverStatus.AVAILABLE) {
                ...
                if (distance <= 5.0) { // Notify drivers within 5 Unit radius
    ...
    private void notifyDriver(Ride ride) {
        ...
        if (driver != null) {
            ...
            // Send notification to the driver
            System.out.println("Notifying driver " + driver.getName() + " about " + message);
        }
    }
    ...
    public double calculateDistance(Location source, Location destination) {
        ...
        // For simplicity, Let's assume a random distance between 1 and 20
        return Math.random()*20 + 1;
    }

    private double calculateDuration(Location source, Location destination) {
        // Calculate the estimated duration between two locations based on distance and distance and average speed
